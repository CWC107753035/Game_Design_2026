...
namespace Slime
{
    ...
    public static class PBF_Utils
    {
        ...
        public const int BubblesCount = 128;
    ...
    {
        [BurstCompile]
        public struct RecordOffsetsJob : IJobParallelFor
        {
            [ReadOnly] public NativeArray<Particle> Ps;
            [WriteOnly] public NativeArray<float3> FrozenOffsets;
            public float3 Center;
            public quaternion Rotation;

            public void Execute(int i)
            {
                float3 offset = Ps[i].Position - Center;
                FrozenOffsets[i] = math.mul(math.inverse(Rotation), offset);
            }
        }
        ...
        public struct ApplyForceJob : IJobParallelFor
        {
            ...
            public float3 Center;
            public quaternion Rotation;
            [ReadOnly] public NativeArray<float3> FrozenOffsets;
            ...
            public void Execute(int i)
            {
                ...
                if (IsFrozen)
                {
                    float3 targetPos = Center + math.mul(Rotation, FrozenOffsets[i]);
                    p.Position = math.lerp(p.Position, targetPos, FreezeFactor);
                    velocity = math.lerp(velocity, float3.zero, FreezeFactor);
                }
            ...
            public void Execute(int i)
            {
                ...
                for (int dx = -1; dx <= 1; ++dx)
                {
                    ...
                    for (int j = range.x; j < range.y; j++)
                    {
                        ...
                        float r = math.max(1e-4f, math.sqrt(r2));
                        float3 normDir = r2 > 1e-6f ? dir / r : new float3(1, 0, 0);

                        rho += PBF_Utils.SmoothingKernelPoly6(r2) / PBF_Utils.TargetDensity;
                        float3 grad_j = PBF_Utils.DerivativeSpikyPow3(r) / PBF_Utils.TargetDensity * normDir;
            ...
            public void Execute(int i)
            {
                ...
                for (int dx = -1; dx <= 1; ++dx)
                {
                    ...
                    for (int j = range.x; j < range.y; j++)
                    {
                        ...
                        float r = math.max(1e-4f, math.sqrt(r2));
                        float3 normDir = r2 > 1e-6f ? dir / r : new float3(1, 0, 0);
                        float3 w_spiky = PBF_Utils.SpikyKernelPow3(r) * normDir;
        ...
        public struct ApplyViscosityJob : IJobParallelFor
        {
            ...
            public void Execute(int i)
            {
                ...
                // Limit the viscosity effect to prevent explicit integration oscillation and explosion
                float maxViscosity = math.min(0.48f, ViscosityStrength * PBF_Utils.DeltaTime);
                VelocityW[i] = vel + viscosityForce / PBF_Utils.TargetDensity * maxViscosity;
            }
        }
    }
}